...
def get_args():
    ...
    group.add_argument('--hf_dataset_name', type=str, help='Dataset name to use for training')
    
    group.add_argument('--hf_subset', type=str, help='Dataset subset to use for training')
    group.add_argument('--checkpoint-dir', type=str, help='Path to store files')
    
    group.add_argument('--dataset_split', type=str, help='Dataset split to use for training')
    group.add_argument('--token', type=str, help='Huggingface token to use to save llama weights')
    group.add_argument('--model_id', type=str, help="Huggingface model to use for tokenization")
...
def main():
    args = get_args()
    print("******listing files in the fsx dir *****************")
    print(os.system("ls /opt/ml/input/data/train"))
    startup_start = time.time()
    if args.model_id:
        HfFolder.save_token(args.token)
        if not os.path.exists(args.tokenizer_type):
            os.mkdir(args.tokenizer_type)
        snapshot_download(repo_id=args.model_id, allow_patterns=["*.json","*.model"], ignore_patterns=["*.safetensors","*.safetensors.index.json"],local_dir=args.tokenizer_type,local_dir_use_symlinks=False)
    
    if args.hf_dataset_name:
        ds = load_dataset(args.hf_dataset_name,args.hf_subset,split=args.dataset_split,cache_dir="/tmp")
        #for _, dataset in ds.items():
        ds.to_json(args.input)
